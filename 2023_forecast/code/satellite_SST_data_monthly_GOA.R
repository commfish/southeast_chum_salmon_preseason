# Satellite SST variables for chum salmon forecasting
...
sst_path <- paste0(out.label, "/data/satellite_SST/GOA/")
if(!exists(sst_path)){dir.create(sst_path)}
#------------------------------------------------------------------------------------------------------------------------------------------
# DOWNLOAD SST DATA FROM THE NOAA SITE AND CREATE CSV FILE FOR YEARS 1985 to 2021 (April - July data)
#------------------------------------------------------------------------------------------------------------------------------------------
# 1977-2021 data----
...
# set the latitude (71, 50) and longitude (-160, -130) and time period (April 1985 - July 2021), and fill type as .nc,
# and then paste the URL in the section below
#https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW_monthly.graph?sea_surface_temperature%5B(2020-06-15T23:00:00Z):1:(2020-06-15T23:00:00Z)%5D%5B(71.025):1:(50.025)%5D%5B(-159.975):1:(-129.975)%5D,mask%5B(2020-06-15T23:00:00Z):1:(2020-06-15T23:00:00Z)%5D%5B(71.025):1:(50.025)%5D%5B(-159.975):1:(-129.975)%5D,sea_surface_temperature_anomaly%5B(2020-06-15T23:00:00Z):1:(2020-06-15T23:00:00Z)%5D%5B(71.025):1:(50.025)%5D%5B(-159.975):1:(-129.975)%5D
# https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW_monthly.graph; this site is helpful to map the area of interest

# Depending on the dates and the latitude and longitude chosen, the site will timeout after 60 seconds. Therefore, download the data instead as a nc file (with the lat/long and time period as line 33),
# save it to the data folder and skip lines 37-38 below
# download.file(url = paste0("https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW_monthly.htmlTable?sea_surface_temperature%5B(1970-04-15T23:00:00Z):1:(2021-06-15T23:00:00Z)%5D%5B(60):1:(54)%5D%5B(-137.2):1:(-130)%5D,mask%5B(1970-04-15T23:00:00Z):1:(2021-06-15T23:00:00Z)%5D%5B(60):1:(54)%5D%5B(-137.2):1:(-130)%5D,sea_surface_temperature_anomaly%5B(1970-04-15T23:00:00Z):1:(2021-06-15T23:00:00Z)%5D%5B(60):1:(54)%5D%5B(-137.2):1:(-130)%5D"),
#              method = "libcurl", mode="wb", destfile = (paste0(sst_path,'NOAA_DHW_monthly_97_21.nc'))0
...
# download April 1985 to July 2000 data
tidync(paste0(sst_path,'NOAA_DHW_monthly_85_00.nc')) %>% 
  hyper_tibble() %>% 
...
# download April 2001 to July 2020 data
tidync(paste0(sst_path,'NOAA_DHW_monthly_01_20.nc')) %>% 
  hyper_tibble() %>% 
  mutate(date=lubridate::as_datetime(time)) %>%
  group_by(date, latitude, longitude) %>% 
  summarise(SST=round(mean(sea_surface_temperature),2)) %>%
...
# If the data is not available for the current months (as was in 2021), the daily data for April-July 2021 was downloaded into a nc file and then
# combined with the monthly data from 1985 through 2020. Otherwise, if all months are available,
...
tidync(paste0(sst_path,'NOAA_CRW_daily_21.nc')) %>% 
  hyper_tibble() %>% 
  mutate(date=lubridate::as_datetime(time)) %>%
  group_by(date, latitude, longitude) %>% 
...
rbind(SST_satellite1, SST_satellite2) %>%  # combine data from April 1985 through June 2021 with the July 2021 data file
rbind(., SST_satellite3) -> SST_satellite
# SST_satellite %>%
#   write.csv(., paste0(sst_path, 'sst_oisst_1985_2021_monthly_data.csv')) # final data set
...
read.csv(paste0(sst_path, 'sst_data_map.csv')) %>% 
  rename(latitude = Y,
         longitude= X) %>% 
...
merge(SST_MJJ, SST_May, by = c("region", "year")) %>%
  ...
  merge(., SST_AMJ, by = c("region", "year")) -> SST_regions_data  
...
tempdata %>%
  ...
  dplyr::select(year, Icy_Strait_SST_MJJ, Icy_Strait_SST_May, Icy_Strait_SST_AMJJ, Icy_Strait_SST_AMJ) -> Icy_Strait_summary
...
tempdata %>%
  ...
  dplyr::select(year, Chatham_SST_MJJ, Chatham_SST_May, Chatham_SST_AMJJ, Chatham_SST_AMJ) -> Chatham_Strait_summary
...
tempdata %>%
  ...
  dplyr::select(year, NSEAK_SST_MJJ, NSEAK_SST_May, NSEAK_SST_AMJJ, NSEAK_SST_AMJ) -> NSEAK_summary
...
tempdata %>%
  ...
  dplyr::select(year, SEAK_SST_MJJ, SEAK_SST_May, SEAK_SST_AMJJ, SEAK_SST_AMJ) -> SEAK_summary

#---------------------------------------------------------------------------------------------------------------------------------------------
# COMBINE DATA SETS FOR FIG
#---------------------------------------------------------------------------------------------------------------------------------------------
merge (Icy_Strait_summary, Chatham_Strait_summary) %>%
merge (., NSEAK_summary) %>% 
merge (., SEAK_summary) -> fig_data
...
fig_data %>%
  ...
  scale_x_continuous(breaks = 1985:2021, labels = 1985:2021) +
  scale_y_continuous(breaks = c(6,7, 8, 9,10,11,12), limits = c(6,12))+
  geom_text(aes(x = 1989, y = 12, label="A) May temperature"),family="Times New Roman", colour="black", size=4) +
...
fig_data %>%
  ...
  scale_x_continuous(breaks = 1985:2021, labels = 1985:2021) +
  scale_y_continuous(breaks = c(6,7, 8, 9,10, 11, 12), limits = c(6,12))+
  geom_text(aes(x = 1992, y = 12, label="B) May, June, July temperature"),family="Times New Roman", colour="black", size=4) +
...
fig_data %>%
  ...
  scale_x_continuous(breaks = 1985:2021, labels = 1985:2021) +
  scale_y_continuous(breaks = c(6,7, 8, 9,10, 11,12), limits = c(6,12))+
  geom_text(aes(x = 1993.5, y = 12, label="D) April, May, June, July temperature"),family="Times New Roman", colour="black", size=4) +
...
fig_data %>%
  ...
  scale_x_continuous(breaks = 1985:2021, labels = 1985:2021) +
  scale_y_continuous(breaks = c(6,7, 8, 9,10, 11,12), limits = c(6,12))+
  geom_text(aes(x = 1992, y = 12, label="C) April, May, June temperature"),family="Times New Roman", colour="black", size=4) +
  labs(y = "Temperature (Celsius)", x ="") -> plot4
cowplot::plot_grid(plot1,plot2,plot4, plot3, align = "v", nrow = 4, ncol=1)
ggsave(paste0(sst_path, "monthly_temp_regions.png"), dpi = 500, height = 8, width = 7, units = "in")
...
read.csv(file.path(sst_path,'SST_data_map.csv'), header=TRUE, as.is=TRUE, strip.white=TRUE) -> SST_data
SST_data %>%
...
png(paste0(sst_path, "Icy_Strait.png"),width=6,height=8,units="in", res=600)                                                        
...
region<-clipPolys(nepacLLhigh,xlim=x,ylim=y)      
par(mfrow=c(1,1),omi=c(0,0,0,0))  
png(paste0(sst_path, "Chatham.png"),width=6,height=8,units="in", res=600)                                                        
plotMap(region,xlim=x, ylim=y, tck=-0.02, plt=c(.13,.98,.13,.98),projection="LL", cex=1.2,
        xlab="Longitude (°W)", ylab="Latitude (°N)", cex.lab=1.5, font.lab=6, col=clr$land, bg=clr$sea)
...
region<-clipPolys(nepacLLhigh,xlim=x,ylim=y)      
par(mfrow=c(1,1),omi=c(0,0,0,0))  
png(paste0(sst_path, "NSEAK.png"),width=6,height=8,units="in", res=600)                                                        
plotMap(region,xlim=x, ylim=y, tck=-0.02, plt=c(.13,.98,.13,.98),projection="LL", cex=1.2,
        xlab="Longitude (°W)", ylab="Latitude (°N)", cex.lab=1.5, font.lab=6, col=clr$land, bg=clr$sea)
...
region<-clipPolys(nepacLLhigh,xlim=x,ylim=y)      
par(mfrow=c(1,1),omi=c(0,0,0,0))  
png(paste0(sst_path, "SEAK.png"),width=6,height=8,units="in", res=600)                                                        
plotMap(region,xlim=x, ylim=y, tck=-0.02, plt=c(.13,.98,.13,.98),projection="LL", cex=1.2,
        xlab="Longitude (°W)", ylab="Latitude (°N)", cex.lab=1.5, font.lab=6, col=clr$land, bg=clr$sea)
