...
data.directory <- paste0(out.label, "/data/satellite_SST/GOA/")
if(!exists(data.directory)){dir.create(data.directory)}
results.directory <- paste0(out.label, "/results/satellite_SST/GOA/")
#-------------------------------------------------------------------------------------------------------------------------------------------
...
SST_data %>%
  filter(region == "GOA") -> GOA

GOA <- as.PolyData(GOA, projection="LL") # GOA

x<-c(-159, -129)   #coordinates of land data
y<-c(54,62)

# test map
region<-clipPolys(nepacLLhigh,xlim=x,ylim=y)      
par(mfrow=c(1,1),omi=c(0,0,0,0))  
png(paste0(results.directory, "test_map.png"),width=10,height=12,units="in", res=600)                                                        
...
addPoints(GOA, xlim=x,ylim=y,col=1,pch=1, lwd=0.6, cex=0.03) # satellite SST data
...
legend (-133, 58.8, legend=c("SST locations"),
        col=c(1,2), bty="n", cex=1, pch=c(1, 16))
text(-145,59,"Gulf of Alaska", cex=1.25, adj=1, font=0.75)
