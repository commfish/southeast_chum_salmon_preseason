...
sst_path <- paste0(out.label, "/data/satellite_SST/SEAK/")
if(!exists(sst_path)){dir.create(sst_path)}
#------------------------------------------------------------------------------------------------------------------------------------------
# DOWNLOAD SST DATA FROM THE NOAA SITE AND CREATE CSV FILE FOR YEARS 1985 to 2022 (April - July data)
#------------------------------------------------------------------------------------------------------------------------------------------
# 1985-2021 data----
# option #1: download the data for a fixed spatial and temporal period	(SST data, NOAA Global Coral Bleaching Monitoring, 5km, V.3.1, monthly, 1985-Present)
# to do this, go to the site: https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW_monthly.html;
# set the latitude (54, 60) and longitude (-137.2, -130) and time period (April 1985 - July 2021), and file type as .nc from the drop down menu
# and select submit; this will create a large file; put this file in the sst_path created above and rename the file to match code line 45
...
# to do this, go to the site: https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW.html
# set the latitude (54, 60) and longitude (-137.2, -130) and time period (April 1 2022 - July  31 2022),and file type as .nc from the drop down menu
# and select submit; this will create a large file; put this file in the sst_path created above and rename the file to match code line 65

# https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW_monthly.graph; this site is helpful to map the area of interest

# download April 1985 to July 2021 data
tidync(paste0(sst_path,'NOAA_DHW_monthly_85_21.nc')) %>% 
  hyper_tibble() %>% 
...
# If the data is not available for the current months (as was in 2022), the daily data for April-July 2022 was downloaded into a nc file and then
...
tidync(paste0(sst_path,'NOAA_CRW_daily_22.nc')) %>% 
  hyper_tibble() %>% 
  mutate(date=lubridate::as_datetime(time)) %>%
  group_by(date, latitude, longitude) %>% 
  ...
  as.data.frame()  %>%
rbind(., SST_satellite1) -> SST_satellite  # combine data from April 1985 through June 2021 with the July 2021 data file

SST_satellite %>%
  write.csv(., paste0(sst_path, 'sst_oisst_97_22_monthly_data.csv')) # final data set
...
rbind(Icy_Strait_SST, NSEAK_SST) %>%
  rbind(., SEAK_SST) %>%   
  rbind(., Chatham_SST) %>% 
  write.csv(., paste0(sst_path, 'sst_regions_oisst_97_22_monthly_data.csv'))
#--------------------------------------------------------------------------------------------------------------------------------------------
# SUMMARIZE SST DATA BY REGION, YEAR, MONTH
#--------------------------------------------------------------------------------------------------------------------------------------------
# summarize SST data
read.csv(paste0(sst_path, 'sst_regions_oisst_97_22_monthly_data.csv')) %>%
...
merge(SST_MJJ, SST_May, by = c("region", "year")) %>%
  ...
  write.csv(., paste0(sst_path, 'sst_regions_oisst_97_22_monthly_data_summary.csv'))

# create tables by region for the report
read.csv(paste0(sst_path, 'sst_regions_oisst_97_22_monthly_data_summary.csv')) %>%
...
tempdata %>%
  ...
  write.csv(., paste0(sst_path, 'sst_oisst_97_22_Icy_Strait_monthly_summary.csv'), row.names = FALSE)
...
tempdata %>%
  ...
  write.csv(., paste0(sst_path, 'sst_oisst_97_22_Chatham_monthly_summary.csv'), row.names = FALSE)
...
tempdata %>%
  ...
  write.csv(., paste0(sst_path, 'sst_oisst_97_22_NSEAK_monthly_summary.csv'), row.names = FALSE) # row.name = F gets rid of the first column X1 that is not needed
...
tempdata %>%
  ...
  write.csv(., paste0(sst_path, 'sst_oisst_97_22_SEAK_monthly_summary.csv'), row.names = FALSE) # final satellite data by month (copy and paste to var*yyyy*_final.csv file)

#---------------------------------------------------------------------------------------------------------------------------------------------
# COMBINE DATA SETS FOR FIG
#---------------------------------------------------------------------------------------------------------------------------------------------
read.csv(paste0(sst_path, 'sst_oisst_97_22_Icy_Strait_monthly_summary.csv')) -> Icy_Strait
read.csv(paste0(sst_path, 'sst_oisst_97_22_Chatham_monthly_summary.csv')) -> Chatham
read.csv(paste0(sst_path, 'sst_oisst_97_22_NSEAK_monthly_summary.csv')) -> NSEAK
read.csv(paste0(sst_path, 'sst_oisst_97_22_SEAK_monthly_summary.csv')) %>% # SEAK region
  merge (., Icy_Strait) %>%
  merge (., Chatham) %>% 
...
fig_data %>%
  ...
  scale_x_continuous(breaks = 1985:2022, labels = 1985:2022) +
